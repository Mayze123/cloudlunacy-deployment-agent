...
class TemplateHandler {
    ...
    async generateDeploymentFiles({ appType, appName, environment, containerPort, domain, envFile, health }) {
        const config = this.deployConfig[appType.toLowerCase()];
        if (!config) {
            throw new Error(`No template found for appType: ${appType}`);
        }

        const dockerfileTemplatePath = path.join(this.templatesDir, config.dockerfileTemplate);
        const dockerComposeTemplatePath = path.join(this.templatesDir, config.dockerComposeTemplate);
        ...
        const dockerfileTemplateContent = await fs.readFile(dockerfileTemplatePath, 'utf-8');
        const dockerComposeTemplateContent = await fs.readFile(dockerComposeTemplatePath, 'utf-8');
