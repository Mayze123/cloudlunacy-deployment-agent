...
check_args() {
  if [ "$#" -lt 2 ] || [ "$#" -gt 3 ]; then
    ...
    echo "  INSTALL_NIXPACKS=true    # Install Nixpacks for optimized container builds"
    exit 1
  fi
}
...
install_nixpacks() {
  if [ "${INSTALL_NIXPACKS:-false}" = "true" ]; then
    ...
    if command -v nixpacks &> /dev/null; then
      log "Nixpacks is already installed. Version: $(nixpacks --version)"
      return 0
    fi
    ...
    if command -v npm &> /dev/null; then
      ...
      if command -v nixpacks &> /dev/null; then
        log "Nixpacks installed successfully with npm!"
        return 0
      else
        log_warn "Failed to install Nixpacks with npm. Trying alternative methods..."
      fi
    fi
    ...
    if command -v curl &> /dev/null; then
      ...
      if command -v nixpacks &> /dev/null; then
        log "Nixpacks installed successfully with curl!"
        return 0
      else
        log_warn "Failed to install Nixpacks with curl. Trying alternative methods..."
      fi
    fi
    ...
    if command -v brew &> /dev/null; then
      ...
      if command -v nixpacks &> /dev/null; then
        log "Nixpacks installed successfully with Homebrew!"
        return 0
      else
        log_warn "Failed to install Nixpacks with Homebrew."
      fi
    fi
...
EOF
      ...
      if nixpacks --version &> /dev/null; then
        ...
        log_warn "Failed to set up Nixpacks Docker wrapper."
      fi
    fi

    log_warn "Failed to install Nixpacks through any method. Applications will fall back to Dockerfile builds."
    # We don't exit with error as this is optional
    return 1
  else
...
main() {
  ...
  # Optionally install Nixpacks for better application builds
  install_nixpacks
...
USE_NIXPACKS=${INSTALL_NIXPACKS:-false}
